...
export default function DailyDeals() {
  ...
  useEffect(() => {
    const calculateTimeLeft = () => {
      ...
      if (dealsConfig?.endTime) {
        targetTime = new Date(dealsConfig.endTime).getTime();
      } else {
      ...
      <div className="container mx-auto px-6 md:px-12">
        ...
        >
          ...
          <div className="space-y-3 max-w-xl z-10">
            <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-wellness-green text-wellness-navy text-xs font-black uppercase tracking-wider">
              ...
              <span>
                {dealsConfig?.discountText ||
                  (dealsConfig?.discountPercentage
                    ? `Save ${String(dealsConfig.discountPercentage)}% Today`
                    : 'Limited Time Offers')}
              </span>
            </div>
        ...
        <div className="deals-grid grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {productsList.map((product, index) => {
            ...
            const mrp =
              configuredPct && configuredPct > 0
                ? Math.round(sp / (1 - configuredPct / 100))
                : product.mrp && product.mrp > sp
                  ? product.mrp
                  : Math.round(sp * 1.2);
            const discount =
              configuredPct && configuredPct > 0
